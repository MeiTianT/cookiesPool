...
#实现：获取账号信息并模拟登录，随后生成cookies并保存
#实现逻辑：首先获取两个Hash信息（account:weibo/cookies:weibo)
#比较account的hash比cookies的hash多出哪些还没有生成cookies的账号
#遍历多出来的账号，生成cookies
#人话：找出还没有生成cookies的账号生成cookies
class CookiesGenerator(object):
    ...
    def run(self):
        ...
        :return:
        """
        #accounts:weibo:username
        accounts_usernames = self.accounts_db.usernames()
        # cookies:weibo:username
        cookies_usernames = self.cookies_db.usernames()

        for username in accounts_usernames:
            #如果accounts里面存在账号还没有生成cookies
...
class WeiboCookiesGenerator(CookiesGenerator):
    ...
    def new_cookies(self, username, password):
        ...
        :param username: 用户名
        :param password: 密码
        :return: 用户名和Cookies/ status,content
        """
